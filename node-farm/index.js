...
const server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    ...
    case '/index.html':
    case '/overview':
      fs.readFile(`${__dirname}/template-overview.html`, 'utf-8', (err, buffer) => {
        if (err) {
          console.log(err);
        ...
        const temps = objs.map((obj) => {
          return replaceTemplate(tempCards, obj);
        });
        const slugs = objs.map((obj) => slugify(obj.productName, { lower: true }));
        console.log(slugs);
